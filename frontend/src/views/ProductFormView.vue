	...
	<form class="product-form">
		<div class="product-form-image">
			<div class="input input-file">
				<div class="input">
					...
					<input type="url" placeholder="Image URL" name="image" id="image" v-model="formData.image" />
				</div>
				<div id="preview">
		...
		<div class="product-form-info">
			<div class="input">
				...
				<input type="text" placeholder="Name" name="name" id="name" v-model="formData.name" />
			</div>
			<div class="input">
				<label for="price">Price</label>
				<input type="number" placeholder="Price" name="price" id="price" step="0.01" v-model="formData.price" />
			</div>
			
			<button class="primary-btn" :disabled="loading" @click.prevent="submitProduct">Crear</button>
		</div>
  	</form>
	<Alert ref="alerts"/>
</template>
	...
	methods: {
		...
		async submitProduct() {
			...
			try {
				...
				await axios.post('https://bouquets.herokuapp.com/bouquets', formData)
					.then((res) => {
						let alert = {
							message: 'Test message',
							color: 'success',
							action_text: 'View',
							action: () => { this.$router.push({ name: 'Product', params: { id: res.data.id }})}
						};
